...
from db import BaseDeDatos, ContactoRepository, PerfilRepository, ConfiguracionRepository, AutomatizacionRepository, CategoriaRepository, TareaRepository, EventoRepository, RecordatorioPanelRepository, OrganizacionRepository, PanelResumenRepository, NotaRepository, FavoritoHistorialHelper, FavoritoRepository, HistorialGeneralRepository, SuscripcionRepository, NotificadorCorreo, CorreoRepository, ArchivoRepository
app = Flask(__name__)
CORS(app)
bd = BaseDeDatos()
...
correos_repo = CorreoRepository(bd)
archivos_repo = ArchivoRepository(bd)
...
def login():
    ...
    resultado = bd.ejecutar('SELECT usuario, nombre_completo, alias FROM usuarios\n           WHERE usuario = %s AND contrasena_hash = crypt(%s, contrasena_hash)', (usuario, contrasena), retornar='one')
    if not resultado:
...
def actualizar_contacto(contacto_id):
    datos = request.get_json(force=True)
    resultado = contactos_repo.actualizar_flags(contacto_id, oculto=datos.get('oculto'), archivado=datos.get('archivado'))
...
@app.put('/api/contactos/<contacto_id>')
def modificar_contacto(contacto_id):
    ...
    if not nombre or not telefono:
        return error('Nombre y teléfono son obligatorios.')
    return jsonify(contactos_repo.modificar(contacto_id, nombre, telefono))
...
TIPOS_REGLA_VALIDOS = {'bidireccional', 'direccional', 'solo_si_se_cumple_condicion', 'depurador_detallado', 'solo_si_no_se_cumple_condicion'}
...
def organizacion_guardar_evento():
    ...
    evento = eventos_repo.guardar(evento_id=datos.get('id') or None, titulo=titulo, descripcion=datos.get('descripcion', ''), fecha=datos.get('fecha'), ubicacion=datos.get('ubicacion', ''))
    if es_nuevo:
...
def panel_crear_evento():
    ...
    nuevo = eventos_repo.crear_detallado(titulo=titulo, descripcion=datos.get('descripcion', ''), ubicacion=datos.get('ubicacion', ''), fecha_inicio=fecha_inicio, fecha_fin=fecha_fin)
...
def panel_crear_tarea():
    ...
    nueva = tareas_repo.crear(titulo=titulo, descripcion=datos.get('descripcion', ''), prioridad=datos.get('prioridad', 'media'), fecha_vencimiento=datos.get('fecha_vencimiento'))
...
def panel_actualizar_flags_recordatorio(recordatorio_id):
    datos = request.get_json(force=True)
    resultado = recordatorios_panel_repo.actualizar_flags(recordatorio_id, oculto=datos.get('oculto'), archivado=datos.get('archivado'))
    if resultado is None:
        return error('No se envió ningún campo para actualizar (oculto/archivado).')
    return jsonify(resultado)
...
def actividades_resumen():
    return jsonify({'nota_actual': notas_repo.obtener_activa(), 'todas_las_notas': notas_repo.listar()})
...
def informacion_crear_favorito():
    ...
    nuevo = favoritos_repo.crear(titulo, tipo_elemento=datos.get('tipo_elemento', 'general'), elemento_id=datos.get('elemento_id'))
    return (jsonify(nuevo), 201)
...
@app.get('/api/actividades/correos')
def actividades_listar_correos():
    cat = request.args.get('categoria', 'todos')
    return jsonify(correos_repo.listar(categoria=cat))

@app.post('/api/actividades/correos')
def actividades_crear_correo():
    datos = request.get_json(force=True)
    destinatario = (datos.get('destinatario') or '').strip()
    asunto = (datos.get('asunto') or '').strip()
    cuerpo = (datos.get('cuerpo') or '').strip()
    remitente = datos.get('remitente', 'Equipos de Gmail')
    categoria = datos.get('categoria', 'principal')
    return (jsonify(correos_repo.crear(destinatario, asunto, cuerpo, remitente, categoria)), 201)

@app.post('/api/actividades/correos/destacado')
def actividades_destacar_correo():
    datos = request.get_json(force=True)
    correo_id = datos.get('id')
    destacado = datos.get('destacado', True)
    return jsonify(correos_repo.toggle_destacado(correo_id, destacado))

@app.post('/api/actividades/correos/eliminar')
def actividades_eliminar_correo():
    datos = request.get_json(force=True)
    correo_id = datos.get('id')
    return jsonify(correos_repo.eliminar(correo_id))

@app.get('/api/actividades/archivos')
def actividades_listar_archivos():
    cat_id = request.args.get('carpeta_id')
    return jsonify(archivos_repo.listar(cat_id))

@app.post('/api/actividades/archivos')
def actividades_crear_archivo():
    datos = request.get_json(force=True)
    nombre = (datos.get('nombre') or '').strip()
    cat_id = datos.get('carpeta_id')
    return (jsonify(archivos_repo.crear(nombre, categoria_id=cat_id)), 201)

@app.post('/api/actividades/archivos/eliminar')
def actividades_eliminar_archivo():
    datos = request.get_json(force=True)
    archivo_id = datos.get('archivo_id') or datos.get('id')
    return jsonify(archivos_repo.eliminar(archivo_id))

@app.post('/api/informacion/suscripciones/pago')
def informacion_pago_suscripcion():
    datos = request.get_json(force=True)
    sub_id = datos.get('id')
    monto = datos.get('monto', 9.99)
    fecha = datos.get('fecha')
    return jsonify(suscripciones_repo.registrar_pago(sub_id, monto, fecha))
...
def informacion_resumen():
    ...
    return jsonify({'favoritos': favoritos, 'historial': historial, 'suscripciones': suscripciones, 'estadisticas': {'total_favoritos': len(favoritos), 'total_historial': len(historial), 'total_suscripciones': len(suscripciones), 'suscripciones_activas': len([s for s in suscripciones if s.get('activa')])}})
