...
class AutomatizacionRepository:
    ...
    def historial(self):
        return self.bd.ejecutar(
            """SELECT h.id, h.titulo, h.fecha_ejecucion, h.estado,
                      a.id AS automatizacion_id, a.nombre AS automatizacion_nombre, a.activa
    ...
    def eliminar_historial(self, historial_id):
        self.bd.ejecutar(
            "DELETE FROM historial_automatizacion WHERE id = %s", (historial_id,), retornar="none"
        )

    def toggle_activa(self, automatizacion_id):
        actual = self.bd.ejecutar(
            "SELECT activa FROM automatizaciones WHERE id = %s", (automatizacion_id,), retornar="one"
        )
        nueva = (not actual["activa"]) if actual else True
        return self.bd.ejecutar(
            "UPDATE automatizaciones SET activa = %s WHERE id = %s RETURNING id, activa",
            (nueva, automatizacion_id),
            retornar="one",
        )
