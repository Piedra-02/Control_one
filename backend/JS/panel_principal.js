...
function initTaskCheckboxes() {
    ...
    checkboxes.forEach(checkbox => {
        checkbox.addEventListener('change', (e) => {
            ...
            if (taskId) {
                fetch('http://localhost:8000/api/panel/tareas/estado', {
                    method: 'POST',
...
function guardarTareaModal(e) {
    ...
    if (id) {
        ...
        newCard.className = 'task-item-card';
        newCard.setAttribute('data-task-id', newId);
        newCard.innerHTML = `
            ...
            <span class="task-item-text">${escapeHTML(texto)}</span>
        `;
        ...
        // Enviar a Python
        fetch('http://localhost:8000/api/panel/tareas', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
function guardarEventoModal(e) {
    ...
    if (eventCard) {
        ...
        const descLines = desc.split(' ').map(w => `<p>${escapeHTML(w)}</p>`).join('');
        const descCol = eventCard.querySelector('.event-desc-col');
        if (descCol) descCol.innerHTML = descLines || `<p>${escapeHTML(desc)}</p>`;

        const dateCol = eventCard.querySelector('.event-date-text');
        if (dateCol && fecha) {
            const fPartes = fecha.split('-');
            if (fPartes.length === 3) {
                dateCol.textContent = `${fPartes[2]}/${fPartes[1]}/${fPartes[0]}`;
            }
        }
    }

    // Persistir en LocalStorage para sincronizar con Organización
    const nuevoEvento = { id: id || 'e_' + Date.now(), titulo: nombre, descripcion: desc, fecha: fecha };
    guardarEventoEnStorage(nuevoEvento);

    fetch('http://localhost:8000/api/panel/eventos', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        ...
        if (eventCard) {
            eventCard.innerHTML = `
                ...
                <div class="event-card-body">
                    <p style="font-size: 13px; color: #555;">Toca aquí o en el cuadro verde para agregar un evento.</p>
                </div>
            `;
        }
        cerrarModales();
...
async function sincronizarConBackendPython() {
    try {
        const respuesta = await fetch('http://localhost:8000/api/panel/resumen', {
