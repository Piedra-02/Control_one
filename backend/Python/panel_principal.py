...
class PanelPrincipalDB:
    ...
    def _init_mock_data(self):
        self.eventos = [{'id': 'e1', 'titulo': 'Cumpleaños Mamá', 'descripcion': 'Celebración familiar y cena de cumpleaños.', 'ubicacion': 'Casa familiar', 'fecha_inicio': '2026-08-30T19:00:00', 'fecha_fin': '2026-08-30T23:00:00', 'categoria': 'Familia', 'color': '#00bcd4'}]
        self.tareas = [{'id': 't1', 'titulo': 'Hacer las compras del mes', 'descripcion': 'Comprar víveres, artículos de limpieza y despensa general.', 'estado': 'pendiente', 'prioridad': 'alta', 'fecha_vencimiento': '2026-08-30T18:00:00', 'categoria': 'Hogar'}, {'id': 't2', 'titulo': 'Revisar facturas de servicios', 'descripcion': 'Comprobar facturas de luz, agua e internet.', 'estado': 'pendiente', 'prioridad': 'media', 'fecha_vencimiento': '2026-08-31T20:00:00', 'categoria': 'Finanzas'}, {'id': 't3', 'titulo': 'Organizar carpetas de diseño', 'descripcion': 'Ordenar los archivos fuente e ilustraciones en assets.', 'estado': 'completada', 'prioridad': 'baja', 'fecha_vencimiento': '2026-08-29T16:00:00', 'categoria': 'Diseño'}, {'id': 't4', 'titulo': 'Actualizar copia de seguridad', 'descripcion': 'Generar respaldo del esquema de base de datos.', 'estado': 'pendiente', 'prioridad': 'alta', 'fecha_vencimiento': '2026-09-01T12:00:00', 'categoria': 'Sistema'}]
        self.recordatorios = [{'id': 'r1', 'titulo': 'Tomar descanso activo', 'subtitulo': 'Pausa de 5 minutos y estiramientos', 'fecha_hora': '2026-08-31T18:30:00', 'hora_formateada': '18:30', 'destacado': True, 'enviado': False}, {'id': 'r2', 'titulo': 'Sincronizar tareas de la semana', 'subtitulo': 'Planificar las actividades del día de mañana', 'fecha_hora': '2026-08-31T21:00:00', 'hora_formateada': '21:00', 'destacado': False, 'enviado': False}, {'id': 'r3', 'titulo': 'Regar las plantas del balcón', 'subtitulo': 'Cuidado del jardín', 'fecha_hora': '2026-09-01T08:00:00', 'hora_formateada': '08:00', 'destacado': True, 'enviado': False}]
    ...
    def crear_evento(self, titulo, descripcion='', ubicacion='', fecha_inicio=None, fecha_fin=None, categoria_id=None):
        if self.usar_mock:
            nuevo = {'id': f'e{len(self.eventos) + 1}', 'titulo': titulo, 'descripcion': descripcion, 'ubicacion': ubicacion, 'fecha_inicio': fecha_inicio or datetime.now().isoformat(), 'fecha_fin': fecha_fin or datetime.now().isoformat(), 'categoria': 'General', 'color': '#00bcd4'}
    ...
    def crear_tarea(self, titulo, descripcion='', prioridad='media', fecha_vencimiento=None, categoria_id=None):
        if self.usar_mock:
            nueva = {'id': f't{len(self.tareas) + 1}', 'titulo': titulo, 'descripcion': descripcion, 'estado': 'pendiente', 'prioridad': prioridad, 'fecha_vencimiento': fecha_vencimiento or datetime.now().isoformat(), 'categoria': 'General'}
    ...
    def crear_recordatorio(self, titulo, fecha_hora, subtitulo='', tarea_id=None, evento_id=None):
        if self.usar_mock:
            nuevo = {'id': f'r{len(self.recordatorios) + 1}', 'titulo': titulo, 'subtitulo': subtitulo or 'Recordatorio personal', 'fecha_hora': fecha_hora, 'hora_formateada': fecha_hora.split('T')[1][:5] if 'T' in str(fecha_hora) else '12:00', 'destacado': False, 'enviado': False}
    ...
    def buscar(self, query):
        ...
        if self.usar_mock:
            eventos_filtrados = [e for e in self.eventos if q in e['titulo'].lower() or q in e.get('descripcion', '').lower()]
            tareas_filtradas = [t for t in self.tareas if q in t['titulo'].lower() or q in t.get('descripcion', '').lower()]
            recordatorios_filtrados = [r for r in self.recordatorios if q in r['titulo'].lower() or q in r.get('subtitulo', '').lower()]
            return {'eventos': eventos_filtrados, 'tareas': tareas_filtradas, 'recordatorios': recordatorios_filtrados, 'total_coincidencias': len(eventos_filtrados) + len(tareas_filtradas) + len(recordatorios_filtrados)}
        with self.conexion.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
    ...
    def obtener_resumen(self):
        ...
        return {'estadisticas': {'tareas_pendientes': tareas_pendientes, 'tareas_completadas': tareas_completadas, 'total_eventos': len(eventos), 'total_recordatorios': len(recordatorios), 'fecha_actual': datetime.now().strftime('%d/%m/%Y')}, 'eventos': eventos, 'tareas': tareas, 'recordatorios': recordatorios}
...
class PanelPrincipalAPIHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if parsed.path == '/api/panel/tareas':
            nueva = self.db.crear_tarea(titulo=data.get('titulo', ''), descripcion=data.get('descripcion', ''), prioridad=data.get('prioridad', 'media'), fecha_vencimiento=data.get('fecha_vencimiento'))
            self._set_headers(201)
            self.wfile.write(json.dumps(nueva, cls=SerializadorFecha).encode('utf-8'))
        elif parsed.path == '/api/panel/eventos':
            nuevo = self.db.crear_evento(titulo=data.get('titulo', ''), descripcion=data.get('descripcion', ''), ubicacion=data.get('ubicacion', ''), fecha_inicio=data.get('fecha_inicio'), fecha_fin=data.get('fecha_fin'))
            self._set_headers(201)
            self.wfile.write(json.dumps(nuevo, cls=SerializadorFecha).encode('utf-8'))
        elif parsed.path == '/api/panel/recordatorios':
            nuevo = self.db.crear_recordatorio(titulo=data.get('titulo', ''), fecha_hora=data.get('fecha_hora', datetime.now().isoformat()), subtitulo=data.get('subtitulo', ''))
            self._set_headers(201)
            self.wfile.write(json.dumps(nuevo, cls=SerializadorFecha).encode('utf-8'))
