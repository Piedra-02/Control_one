"""
Control One - Backend de Organización
--------------------------------------
Maneja las operaciones y consultas específicas para la vista de Organización:
  - Calendario y gestión de Eventos por fecha.
  - CRUD Completo de Categorías (Crear, Modificar, Eliminar).
  - Tareas por Categoría única (una tarea pertenece a una sola categoría).
  - Estadísticas y métricas de productividad.

Puede ejecutarse directamente:
    python backend/Python/organizacion.py
"""
...
class OrganizacionDB:
    """Acceso a datos para Organización (Calendario, Categorías y Tareas)."""
    ...
    def _init_mock_data(self):
        self.eventos = [
            {
                "id": "e1",
                "titulo": "Cumpleaños Mama",
                "descripcion": "hoy es el cumpleaños de tu Mama",
            ...
            },
            {
                ...
                "descripcion": "Revisión final de Control One",
                "ubicacion": "Oficina",
                "fecha": "2026-08-27",
                "fecha_inicio": "2026-08-27T10:00:00",
                "fecha_fin": "2026-08-27T12:00:00",
                "categoria": "Trabajo",
                "color": "#3b82f6",
                "estado": "pendiente"
            },
            {
                ...
                "descripcion": "Planificar tareas del mes de septiembre",
                "ubicacion": "Virtual",
                "fecha": "2026-08-31",
                "fecha_inicio": "2026-08-31T15:00:00",
                "fecha_fin": "2026-08-31T16:00:00",
                "categoria": "General",
                "color": "#8b5cf6",
                "estado": "pendiente"
            }
        ...
        # Tareas con asignación única de categoría (una tarea pertenece a UNA sola categoría)
        self.tareas = [
    ...
    def obtener_categorias(self):
        """Retorna todas las categorías con sus tareas asociadas."""
        if self.usar_mock:
            resultado = []
            for cat in self.categorias:
                # Filtrar tareas que pertenecen estrictamente a esta categoría
                tareas_cat = [t for t in self.tareas if t.get("categoria_id") == cat["id"]]
                resultado.append({
                    "id": cat["id"],
                    "nombre": cat["nombre"],
                    "color": cat.get("color", "#00bcd4"),
                    "tareas": tareas_cat
                })
            return resultado
    ...
    def eliminar_categoria(self, categoria_id):
        """Elimina la categoría y desvincula sus tareas para no borrarlas."""
        if self.usar_mock:
    ...
    # ---------------------------------------------------------
    # GESTIÓN DE TAREAS Y CATEGORÍA ÚNICA
    # ---------------------------------------------------------
    def asignar_tarea_categoria(self, tarea_id, categoria_id):
        """Asigna una tarea a una única categoría."""
        if self.usar_mock:
            for t in self.tareas:
    ...
    def guardar_evento(self, evento_data):
        ...
        if self.usar_mock:
            ...
            nuevo = {
                ...
                "ubicacion": ubicacion,
                "fecha": fecha,
                "fecha_inicio": f"{fecha}T10:00:00",
                "fecha_fin": f"{fecha}T12:00:00",
                "categoria": "General",
                "color": "#00cba9",
                "estado": "pendiente"
            }
    ...
    def obtener_todo_organizacion(self, anio=2026, mes=8):
        return {
            "mes": mes,
            "anio": anio,
            "nombre_mes": "agosto de 2026",
            "eventos": self.obtener_eventos_mes(anio, mes),
            "categorias": self.obtener_categorias(),
            "estadisticas": {
                "puntos_grafico": [20, 25, 80, 45, 10, 50, 50],
...
# =============================================================
# SERVIDOR API HTTP LOCAL PARA ORGANIZACIÓN
# =============================================================
