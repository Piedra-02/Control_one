...
class InformacionDB:
    ...
    def _init_mock_data(self):
        self.favoritos = [{'id': 'fav1', 'titulo': 'Cumpleaños de Mama', 'tipo_elemento': 'evento', 'elemento_id': 'e1', 'fecha_agregado': '2026-08-30T10:00:00'}, {'id': 'fav2', 'titulo': 'Revision de correo Social de Discord', 'tipo_elemento': 'correo', 'elemento_id': 'c6', 'fecha_agregado': '2026-08-31T12:00:00'}, {'id': 'fav3', 'titulo': 'Suscripcion de Disney', 'tipo_elemento': 'suscripcion', 'elemento_id': 'sub2', 'fecha_agregado': '2026-08-31T14:30:00'}]
        self.historial = [{'id': 'h1', 'hora': '22:01', 'hora_linea1': '22:', 'hora_linea2': '01', 'descripcion': 'Suscripcion de netflix', 'tipo_elemento': 'suscripcion', 'accion': 'creado', 'fecha': '2026-08-31T22:01:00'}, {'id': 'h2', 'hora': '22:06', 'hora_linea1': '22:', 'hora_linea2': '06', 'descripcion': 'Nuevo favorito agregado', 'tipo_elemento': 'favorito', 'accion': 'creado', 'fecha': '2026-08-31T22:06:00'}, {'id': 'h3', 'hora': '22:12', 'hora_linea1': '22:', 'hora_linea2': '12', 'descripcion': 'Recordatorio: Feliz cumpleaños mama', 'tipo_elemento': 'recordatorio', 'accion': 'consultado', 'fecha': '2026-08-31T22:12:00'}, {'id': 'h4', 'hora': '22:33', 'hora_linea1': '22:', 'hora_linea2': '33', 'descripcion': 'Revision de correo', 'tipo_elemento': 'correo', 'accion': 'consultado', 'fecha': '2026-08-31T22:33:00'}, {'id': 'h5', 'hora': '22:56', 'hora_linea1': '22:', 'hora_linea2': '56', 'descripcion': 'Suscripcion de Disney', 'tipo_elemento': 'suscripcion', 'accion': 'editado', 'fecha': '2026-08-31T22:56:00'}]
        self.suscripciones = [{'id': 'sub1', 'nombre': 'Play Station Plus', 'monto': 9.99, 'moneda': 'USD', 'activa': True, 'categoria': 'Gaming', 'metodo_pago': 'Tarjeta Visa **** 4242'}, {'id': 'sub2', 'nombre': 'Directv Go', 'monto': 14.99, 'moneda': 'USD', 'activa': True, 'categoria': 'Streaming TV', 'metodo_pago': 'Tarjeta Mastercard **** 8821'}, {'id': 'sub3', 'nombre': 'DAZN', 'monto': 19.99, 'moneda': 'USD', 'activa': True, 'categoria': 'Deportes', 'metodo_pago': 'Tarjeta Visa **** 4242'}, {'id': 'sub4', 'nombre': 'Spotify', 'monto': 5.99, 'moneda': 'USD', 'activa': True, 'categoria': 'Música', 'metodo_pago': 'PayPal'}, {'id': 'sub5', 'nombre': 'Duolingo Plus', 'monto': 6.99, 'moneda': 'USD', 'activa': True, 'categoria': 'Educación', 'metodo_pago': 'Tarjeta Visa **** 4242'}, {'id': 'sub6', 'nombre': 'Gemini Pro', 'monto': 19.99, 'moneda': 'USD', 'activa': True, 'categoria': 'Inteligencia Artificial', 'metodo_pago': 'Google Pay'}]
    ...
    def crear_favorito(self, titulo, tipo_elemento='general', elemento_id=None):
        if self.usar_mock:
            nuevo = {'id': f'fav_{len(self.favoritos) + 1}_{int(datetime.now().timestamp())}', 'titulo': titulo.strip(), 'tipo_elemento': tipo_elemento, 'elemento_id': elemento_id or f'elem_{int(datetime.now().timestamp())}', 'fecha_agregado': datetime.now().isoformat()}
    ...
    def registrar_historial(self, descripcion, tipo_elemento='general', accion='creado'):
        dt = datetime.now()
        if self.usar_mock:
            nuevo = {'id': f'h_{len(self.historial) + 1}_{int(dt.timestamp())}', 'hora': dt.strftime('%H:%M'), 'hora_linea1': f"{dt.strftime('%H')}:", 'hora_linea2': dt.strftime('%M'), 'descripcion': descripcion.strip(), 'tipo_elemento': tipo_elemento, 'accion': accion, 'fecha': dt.isoformat()}
    ...
    def crear_suscripcion(self, nombre, monto=9.99, moneda='USD', activa=True, categoria='General'):
        nuevo = {'id': f'sub_{len(self.suscripciones) + 1}_{int(datetime.now().timestamp())}', 'nombre': nombre.strip(), 'monto': float(monto), 'moneda': moneda, 'activa': bool(activa), 'categoria': categoria, 'metodo_pago': 'Tarjeta de crédito'}
    ...
    def buscar(self, query):
        ...
        favoritos_filtrados = [f for f in self.favoritos if q in f['titulo'].lower()]
        historial_filtrado = [h for h in self.historial if q in h['descripcion'].lower() or q in h['hora']]
        suscripciones_filtradas = [s for s in self.suscripciones if q in s['nombre'].lower() or q in s.get('categoria', '').lower()]
        return {'favoritos': favoritos_filtrados, 'historial': historial_filtrado, 'suscripciones': suscripciones_filtradas, 'total': len(favoritos_filtrados) + len(historial_filtrado) + len(suscripciones_filtradas)}

    def obtener_resumen(self):
        return {'favoritos': self.obtener_favoritos(), 'historial': self.obtener_historial(), 'suscripciones': self.obtener_suscripciones(), 'estadisticas': {'total_favoritos': len(self.favoritos), 'total_historial': len(self.historial), 'total_suscripciones': len(self.suscripciones), 'suscripciones_activas': len([s for s in self.suscripciones if s.get('activa', True)])}}
...
class InformacionAPIHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if parsed.path == '/api/informacion/favoritos':
            nuevo = self.db.crear_favorito(titulo=data.get('titulo', ''), tipo_elemento=data.get('tipo_elemento', 'general'), elemento_id=data.get('elemento_id'))
            ...
            nueva = self.db.crear_suscripcion(nombre=data.get('nombre', ''), monto=data.get('monto', 9.99), moneda=data.get('moneda', 'USD'), activa=data.get('activa', True), categoria=data.get('categoria', 'General'))
            self._set_headers(201)
